...
| `finance_store.py` + `finance.py` | Discount bank finance (read-only): `FinanceStore` (SQLite: transactions/balance snapshots/category rules, connection-per-op, append-only) + `finance_configured`/`make_collector_fetch` (injectable Node-collector subprocess seam, file-locked, DISCOUNT_* env-scoped) + `build_finance_tools` (`sync_finances`/`financial_summary`/`find_transactions`/`spending_by_category`/`set_category_rule`/`list_category_rules`/`delete_category_rule`/`cash_flow_forecast`). Money is always `Decimal`/agorot-int, never float. Composed in `telegram_app.build_application` only when all three Discount credentials are set. |
