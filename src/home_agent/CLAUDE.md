...
| `schedule_store.py` + `schedules.py` | On-device Bot-timer scheduling: `ScheduleStore` (SQLite record = source of truth, Bots can't be read back, `fire_at` stored UTC) + `build_schedule_tools` (`schedule_device` one-time/`schedule_recurring_device` weekly/`get_schedule`/`cancel_schedule`). |
...
| `finance_store.py` + `finance.py` | Discount bank finance (read-only): `FinanceStore` (SQLite: transactions/balance snapshots/category rules, connection-per-op, append-only) + `finance_configured`/`make_collector_fetch` (injectable Node-collector subprocess seam, file-locked, DISCOUNT_* env-scoped) + `build_finance_tools` (`sync_finances`/`financial_summary`/`find_transactions`/`spending_by_category`/`list_uncategorized_merchants`/`set_category_rule`/`list_category_rules`/`delete_category_rule`/`cash_flow_forecast`). `run_finance_sync(store, fetch_fns, now_fn)` is the plain sync callable shared by the `sync_finances` tool and `telegram_app`'s nightly `job_queue.run_daily(...)` (registered only when finance is configured, at `config.finance_sync_hour`, `ZoneInfo(config.home_tz)`). `_COVERAGE_GRACE_DAYS` (module constant, 3) lets `_spendable_rows` treat a card as covered up to a few days after its last sync — a safety net for a failed nightly sync, not a normal-gap allowance. Money is always `Decimal`/agorot-int, never float. Composed in `telegram_app.build_application` only when all three Discount credentials are set. |
